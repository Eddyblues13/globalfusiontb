...
<div class="mb-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        ...
        <a href="{{ route('cards') }}"
            class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 mt-4 sm:mt-0 w-fit sm:w-auto">
...
@if(session('message'))
<div
    class="rounded-md {{ session('type') == 'success' ? 'bg-green-50 text-green-800 border border-green-100' : 'bg-red-50 text-red-800 border border-red-100' }} p-4 mb-6">
    <div class="flex">
        ...
        <div class="p-6">
            ...
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 mb-6">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    @if($card->status == 'active')
                    <button type="button" id="requestDeliveryBtn"
                        ...
                        <i data-lucide="truck" class="h-4 w-4 mr-1.5"></i> Request Delivery
                    </button>
                    @endif
                    ...
                    <form action="{{ route('cards.block', $card) }}" method="POST" class="inline-block"
                        onsubmit="return confirm('Are you sure you want to block this card? This action may be irreversible.')">
                        @csrf
                        <button type="submit"
                    ...
                    <a href="{{ route('cards.transactions', $card) }}"
            ...
            <div class="w-full max-w-md mx-auto mb-8">
                <div class="credit-card-container perspective-1000">
                    <div class="credit-card" id="creditCard">
                        <!-- Front of the card -->
                        <div
                            class="credit-card-front absolute inset-0 flex flex-col justify-between p-6 backface-hidden">
                            @if($card->card_type == 'visa')
                            <div
                                class="absolute inset-0 bg-gradient-to-br from-blue-800 via-blue-600 to-blue-500 rounded-xl">
                            </div>
                            @elseif($card->card_type == 'mastercard')
                            <div
                                class="absolute inset-0 bg-gradient-to-br from-red-700 via-orange-600 to-orange-500 rounded-xl">
                            </div>
                            @elseif($card->card_type == 'american_express')
                            <div
                                class="absolute inset-0 bg-gradient-to-br from-gray-800 via-gray-700 to-gray-600 rounded-xl">
                            </div>
                            @else
                            <div
                                class="absolute inset-0 bg-gradient-to-br from-gray-800 via-gray-700 to-gray-600 rounded-xl">
                            ...
                            <div class="absolute inset-0 overflow-hidden rounded-xl">
                                <div
                                    class="absolute top-0 right-0 w-40 h-40 bg-white/10 rounded-full -translate-y-20 translate-x-20">
                                </div>
                                <div
                                    class="absolute bottom-0 left-0 w-60 h-60 bg-white/5 rounded-full translate-y-20 -translate-x-20">
                                </div>
                                <div
                                    class="absolute inset-0 backdrop-blur-sm bg-gradient-to-b from-transparent to-black/20">
                                </div>
                            </div>
                            ...
                            <div class="relative flex justify-between items-start">
                                <div>
                                    <div class="text-white font-semibold text-lg tracking-wider">{{ $settings->site_name
                                        }}</div>
                                ...
                                <div>
                                    @if($card->card_type == 'visa')
                                    <svg class="h-8 w-16 text-white" viewBox="0 0 1000 324" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path d="M433.334 318.516H348.568L403.315 5.01855H488.081L433.334 318.516Z"
                                            fill="white" />
                                        <path
                                            d="M727.98 15.9043C712.039 9.56622 686.227 2.51855 653.355 2.51855C574.054 2.51855 519.055 45.5277 518.802 106.826C518.306 152.343 558.126 177.912 587.98 193.316C618.34 209.233 629.315 219.335 629.315 233.014C629.061 254.008 603.493 263.591 579.702 263.591C547.839 263.591 530.7 258.287 505.123 245.892L493.902 240.335L482.166 313.221C501.069 323.07 536.096 331.667 572.643 332.163C657.14 332.163 711.128 289.897 711.895 224.144C712.394 188.508 689.871 160.445 643.015 136.91C615.2 121.744 598.306 111.638 598.554 97.2356C598.554 84.5987 613.224 70.9167 648.747 70.9167C677.823 70.4206 699.053 77.7188 715.707 85.5144L726.681 90.8186L738.417 19.9214L727.98 15.9043Z"
                                            fill="white" />
                                        <path fill-rule="evenodd" clip-rule="evenodd"
                                            d="M841.897 5.01855H773.691C755.782 5.01855 742.397 10.0624 734.862 30.0238L630.236 318.516H715.464C715.464 318.516 729.143 280.806 732.429 272.002C742.901 272.002 822.706 272.002 835.844 272.002C838.373 283.215 845.93 318.516 845.93 318.516H922.667L841.897 5.01855ZM762.22 208.49C769.018 189.752 796.85 116.686 796.85 116.686C796.602 117.174 803.16 99.19 806.699 88.0899L812.02 114.188C812.02 114.188 829.775 192.026 833.061 208.49H762.22Z"
                                            fill="white" />
                                        <path
                                            d="M251.994 5.01855L171.471 219.335L162.137 174.313C146.216 129.054 102.175 80.0894 52.8662 56.8035L126.462 318.02H212.215L336.832 5.01855H251.994Z"
                                            fill="white" />
                                        <path
                                            d="M127.963 5.01855H0.66211L0.167969 10.3101C98.1497 31.7971 163.301 76.5646 190.132 129.298L162.633 13.5747C157.329 -0.60098 144.316 5.51465 127.963 5.01855Z"
                                            fill="white" />
                                    </svg>
                                    @elseif($card->card_type == 'mastercard')
                                    <svg class="h-10 w-16" viewBox="0 0 131.39 86.9" xmlns="http://www.w3.org/2000/svg">
                                        <g opacity=".9">
                                            ...
                                            <path
                                                ...
                                                fill="#fff" />
                                            <path
                                                d="M123.94 43.45a36 36 0 01-58 28.3 36 36 0 000-56.61 36 36 0 0158 28.3z"
                                                fill="#fff" />
                                        </g>
                                    </svg>
                                    @elseif($card->card_type == 'american_express')
                                    <img src="{{ asset('dash/images/cards/amex.png') }}" class="h-8 w-16"
                            ...
                            <div class="relative mt-2">
                                <div
                                    class="w-12 h-10 rounded-md bg-gradient-to-br from-yellow-500 to-yellow-400 flex items-center justify-center overflow-hidden shadow-inner">
                            ...
                            <div class="relative mt-2">
                                <div class="font-mono text-xl text-white tracking-widest drop-shadow-md flex flex-wrap">
                                    ...
                                    <span id="fullCardNumber" class="hidden">{{ chunk_split($card->card_number, 4, ' ')
                                        }}</span>
                                    @endif
                                    @else
                                    <span>•••• •••• •••• ••••</span>
                        ...
                        <!-- Back of the card -->
                        <div
                            class="credit-card-back absolute inset-0 flex flex-col justify-between p-6 backface-hidden rotateY-180">
                            @if($card->card_type == 'visa')
                            <div
                                class="absolute inset-0 bg-gradient-to-br from-blue-700 via-blue-500 to-blue-400 rounded-xl">
                            </div>
                            @elseif($card->card_type == 'mastercard')
                            <div
                                class="absolute inset-0 bg-gradient-to-br from-red-600 via-orange-500 to-orange-400 rounded-xl">
                            </div>
                            @elseif($card->card_type == 'american_express')
                            <div
                                class="absolute inset-0 bg-gradient-to-br from-blue-500 via-blue-400 to-indigo-400 rounded-xl">
                            </div>
                            @else
                            <div
                                class="absolute inset-0 bg-gradient-to-br from-gray-700 via-gray-600 to-gray-500 rounded-xl">
                            </div>
                            @endif
                            ...
                            <div class="absolute inset-0 overflow-hidden rounded-xl">
                                <div
                                    class="absolute top-0 left-0 w-40 h-40 bg-white/10 rounded-full -translate-y-20 -translate-x-20">
                                </div>
                                <div
                                    class="absolute bottom-0 right-0 w-60 h-60 bg-white/5 rounded-full translate-y-20 translate-x-20">
                                </div>
                                <div
                                    class="absolute inset-0 backdrop-blur-sm bg-gradient-to-b from-transparent to-black/20">
                                </div>
                            ...
                            <div class="relative mt-auto">
                                <div class="text-xs text-white/70 space-y-1">
                                    <p>This card is property of {{ $settings->site_name }} and licensed for use by the
                                        authorized cardholder only.</p>
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
    <div class="bg-gray-50 p-4 rounded-lg border border-gray-100 transition-all duration-200 hover:shadow-md">
        ...
        <div class="font-mono text-lg text-gray-800 flex items-center">
            ...
            @if(isset($card->card_number))
            <button type="button" class="ml-2 text-primary-600 hover:text-primary-800 toggle-visibility flex-shrink-0"
                data-target="cardNumberGrid">
    ...
    <div class="bg-gray-50 p-4 rounded-lg border border-gray-100 transition-all duration-200 hover:shadow-md">
        ...
        <div class="text-lg text-gray-800">{{ config('currencies.'.Auth::user()->currency, '$') }}{{
            number_format($card->balance, 2) }}</div>
    </div>
    @endif
</div>
...
<div
    class="bg-white overflow-hidden shadow-sm rounded-xl border border-gray-100 mb-8 transition-all duration-300 hover:shadow-md">
            ...
            <div>
                ...
                <div class="space-y-2">
                    <div class="flex justify-between items-center">
                        ...
                        <span class="text-sm font-medium text-gray-900">{{ config('currencies.'.Auth::user()->currency,
                            '$') }}{{ number_format($card->daily_limit ?? 5000, 2) }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-sm text-gray-600">Monthly Limit:</span>
                        <span class="text-sm font-medium text-gray-900">{{ config('currencies.'.Auth::user()->currency,
                            '$') }}{{ number_format($card->monthly_limit ?? 20000, 2) }}</span>
                    </div>
                    <div class="flex justify-between items-center">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <span class="text-sm font-medium text-gray-900">{{
                            \Carbon\Carbon::parse($card->approval_date)->format('M d, Y') }}</span>
                    </div>
                    @endif
                </div>
            </div>
...
<div
    ...
    <div class="border-b border-gray-200 px-6 py-4 flex justify-between items-center">
        ...
        <a href="{{ route('cards.delivery-history', $card) }}"
    ...
    <div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach($deliveryRequests->take(3) as $request)
                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                        ...
                        <td class="px-6 py-4">
                            ...
                            <div class="text-xs text-gray-500 truncate max-w-xs">{{ $request->address }}</div>
                        </td>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            {{ \Carbon\Carbon::parse($request->updated_at)->format('M d, Y') }}
                        </td>
                    </tr>
                    @endforeach
                </tbody>
        ...
        <div class="py-8 flex flex-col items-center justify-center text-center px-6">
            ...
            <h3 class="text-lg font-medium text-gray-900">No Delivery Requests Yet</h3>
            <p class="text-gray-500 text-sm mt-2 max-w-md">
                You haven't requested delivery for this card yet. Click the "Request Delivery" button to get started.
            </p>
        </div>
        @endif
    </div>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-right">
                            ...
                            <span class="text-red-600">-{{ $card->currency }} {{
                                number_format(abs($transaction->amount), 2) }}</span>
                            @elseif($transaction->transaction_type == 'refund')
                            <span class="text-green-600">+{{ $card->currency }} {{
                                number_format(abs($transaction->amount), 2) }}</span>
                            @else
                            <span class="text-gray-900">{{ $card->currency }} {{ number_format($transaction->amount, 2)
                                }}</span>
                            @endif
                        </td>
                    </tr>
...
<div id="requestDeliveryModal"
    class="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50 hidden transition-opacity duration-300">
    <div
        ...
        <form id="deliveryRequestForm" action="{{ route('cards.delivery.request') }}" method="POST">
            @csrf
            <div class="px-6 py-4 space-y-4">
                ...
                <div>
                    <label for="nearest_airport" class="block text-sm font-medium text-gray-700 mb-1">Nearest Airport
                        (Optional)</label>
                    <input type="text" name="nearest_airport" id="nearest_airport"
                        class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500"
                        placeholder="Helps with international delivery">
                </div>
                <div>
            ...
            <div class="bg-gray-50 px-6 py-3 rounded-b-lg flex justify-end space-x-3">
                ...
                <button type="submit"
                    ...
                    <i data-lucide="truck" class="h-4 w-4 mr-2"></i> Request Delivery
                </button>
            </div>
        </form>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const requestDeliveryModal = document.getElementById('requestDeliveryModal');
        const closeModalBtn = document.getElementById('closeModalBtn');
        const cancelRequestBtn = document.getElementById('cancelRequestBtn');
        const deliveryRequestForm = document.getElementById('deliveryRequestForm');
        
        if (requestDeliveryBtn && requestDeliveryModal) {
            // Open modal
            requestDeliveryBtn.addEventListener('click', function() {
                requestDeliveryModal.classList.remove('hidden');
                setTimeout(() => {
                    requestDeliveryModal.classList.add('modal-open');
                }, 10);
            });
            
            // Close modal functions
            function closeModal() {
                requestDeliveryModal.classList.remove('modal-open');
                setTimeout(() => {
                    requestDeliveryModal.classList.add('hidden');
                }, 300);
            }
            ...
            requestDeliveryModal.addEventListener('click', function(e) {
                if (e.target === requestDeliveryModal) {
                    closeModal();
                }
            });
