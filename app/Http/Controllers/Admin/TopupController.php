    ...
    if ($request['t_type'] == "Credit") {
      if ($request['type'] == "Bonus") {
        ...
        \r\nAccount Name: $user->name $user->middlename $user->lastname
         ...
         \r\nTotal Amount:$request->amount$user->currency
         \r\nDate: $date
         \r\nAvailable Balance:$bala$user->currency";

          $subject = "Credit alert Notification[$request->amount$user->currency]";
        ...
        if ($request['notifymailuser'] == 1) {
          ...
          $message = "Your account have been Debited.
           ...
           \r\nAccount Name: $user->name $user->middlename $user->lastname
            ...
            \r\nDate: $date
            \r\nAvailable Balance: $bala$user->currency";
          $subject = "Debit alert Notification[$request->amount$user->currency]";
          Mail::to($user->email)->send(new NewNotification($message, $subject, $user->name));
