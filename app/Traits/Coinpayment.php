...
trait Coinpayment
{
    ...
    public function paywithcp($amount, $coin, $ui, $msg)
    {
        ...
        if ($msg == 'new') {
            ...
            if ($coin == 'ETH') {
                $payMethod = Wdmethod::where('name', 'Ethereum')->first();
                $address = $payMethod->wallet_address;
            } elseif ($coin == 'BTC') {
                $payMethod = Wdmethod::where('name', 'Bitcoin')->first();
                $address = $payMethod->wallet_address;
            } elseif ($coin == 'LTC') {
                $payMethod = Wdmethod::where('name', 'Litecoin')->first();
                $address = $payMethod->wallet_address;
            } elseif ($coin == 'BUSD') {
                $payMethod = Wdmethod::where('name', 'BUSD')->first();
                $address = $payMethod->wallet_address;
            } else {
                $payMethod = Wdmethod::where('name', 'USDT')->first();
                $address = $payMethod->wallet_address;
            }
                ...
                if ($result['error'] == 'ok') {
                    ...
                    if ($status >= 100 || $status == 2) {
                        ...
                        if ($settings->deposit_bonus != NULL and $settings->deposit_bonus > 0) {
                            $bonus = $txn->amount_paid * $settings->deposit_bonus / 100;
                            //create history
                            Tp_Transaction::create([
                                'user' => $user->id,
                                'plan' => "Deposit Bonus for $settings->currency $txn->amount_paid deposited",
                                'amount' => $bonus,
                                'type' => "Bonus",
                            ]);
                        } else {
                        ...
                        User::where('id', $user->id)
                            ->update([
                                'account_bal' => $user->account_bal + $txn->amount_paid + $bonus,
